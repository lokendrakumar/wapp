...
gulp.task('bower', function () {
  var compileJs = gulp.src(bowerFiles().filter(function (f) { return f.indexOf('.js') > -1; }))
  ...
  var compileCss = gulp.src(bowerFiles().filter(function (f) { return f.indexOf('.css') > -1; }))
...
gulp.task('stylus', function () {
  var buildStyle = function (style) {
    ...
    var g = gulp.src(['stylus/'+style+'.styl'])
                .pipe(stylus())
                .pipe(prefixer({browser: ['last 2 versions'], cascade: false}))
    ;
...
gulp.task('hiring-stylus', function () {
    gulp.src(['stylus/hiring.styl'])
        .pipe(stylus())
        .pipe(prefixer({browser: ['last 2 versions'], cascade: false}))
        .pipe(cssmin()).pipe(dest('css'));
});
